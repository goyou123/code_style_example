...
1. 데이터를 불러와 score기준으로 내림차순으로 정렬하여 5개씩 페이지네이션 합니다.
...
1.  상품수량이 무한정 늘어나는 것을 막기 위해 임의로 최대 입력값을 100으로 설정하였습니다. 추후에는 데이터에 물품의 재고 수량 데이터를 추가하여 각 상품마다 최대 입력값을 재고수량으로 넣을 것 같습니다.
2.  유저들이 헷갈리지 않도록 체크된 상품들이 모두 쿠폰 적용불가 상품이면 할인쿠폰선택이 초기화되고 어떤 쿠폰도 선택할 수 없도록 하였습니다.
3.  장바구니에 아무것도 없으면 결제하는 박스 자체가 보이지 않습니다.
4.  유저들의 편의를 위해 페이지에 다시 들어왔을때 이전에 변경했던 상품의 수량을 그대로 보여줍니다.

<br><br><br>

## 4. 문제 해결 전략

### 4-1) 상태관리를 zustand로 한 이유

> 기존에는 redux로 상태관리를 해왔었는데, 과제 가산점 항목에서 zustand를 보고 조사해본 후 결정한 이유들입니다.
...
2. 해당 과제에는 API연동이 없음
   ...
   그래서 기본적인 부분들만 활용하면 되므로 기간을 고려해보았을때 학습 난이도도 적당하다고 판단하였습니다.
